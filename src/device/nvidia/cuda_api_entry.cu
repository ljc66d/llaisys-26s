...
namespace llaisys::ops::nvidia {
    ...
    void rearrange(std::byte* dst, const std::byte* src, llaisysDataType_t dtype, size_t total_size) {
        throw std::runtime_error("NVIDIA rearrange not implemented yet");
    }
...
void rope(std::byte* dst, const std::byte* src, const std::byte* pos_ids, llaisysDataType_t dtype, size_t batch, size_t seq_len, size_t head_dim, float base, size_t base_dim);
    void self_attention(std::byte* dst, const std::byte* q, const std::byte* k, const std::byte* v, llaisysDataType_t dtype, size_t B, size_t S_q, size_t S_kv, size_t H_q, size_t H_kv, size_t D, float scale);
...
static void wrap_device_synchronize() { CUDA_CHECK(cudaDeviceSynchronize()); }
...
static void wrap_free_device(void* ptr) { if (ptr) cudaFree(ptr); }
static void* wrap_malloc_host(size_t size) {
    void* ptr = nullptr;
    CUDA_CHECK(cudaMallocHost(&ptr, size));
    return ptr;
}
static void wrap_free_host(void* ptr) { if (ptr) cudaFreeHost(ptr); }
...
static void wrap_rearrange(void* dst, const void* src, int dtype, size_t total_size) {
    llaisys::ops::nvidia::rearrange(
        reinterpret_cast<std::byte*>(dst),
        reinterpret_cast<const std::byte*>(src),
        static_cast<llaisysDataType_t>(dtype), total_size);
}
static void wrap_rms_norm(
    void* output,
    const void* input,
    const void* weight,
    int dtype,
...
static void wrap_self_attention(
    ...
    llaisys::ops::nvidia::self_attention(
        reinterpret_cast<std::byte*>(dst),
...
// ========== 唯一导出函数 ==========
extern "C" __declspec(dllexport) llaisys_cuda_api_table_t* llaisys_cuda_get_api_table() {
    return &g_api_table;
}